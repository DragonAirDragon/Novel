...
namespace Naninovel
{
    ...
    public class AudioManager : IStatefulService<SettingsStateMap>, IStatefulService<GameStateMap>, IAudioManager
    {
        ...
        public virtual void SaveServiceState(GameStateMap stateMap)
        {
            ...
            { // Save only looped audio to prevent playing multiple clips at once when the game is (auto) saved in skip mode.
        ...
        public void PlayTranslate(AudioClip clipResource, float volume = 1, string group = null, bool restart = true, bool additive = true)
        {
            audioPlayer.Play(clipResource, null, volume, false, FindAudioGroupOrDefault(group, sfxGroup));
        }
